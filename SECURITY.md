...
- **Entity-Registry (`unique_id`):** SHA-256-Hash der IBAN (mit `entry_id` als Salt), 16 Hex-Zeichen. Die volle IBAN steht damit nicht im aktiven `unique_id`-Feld von `core.entity_registry`. *Restrisiko:* HAs `er.async_migrate_entries` schreibt den Wert vor der Migration zusätzlich nach `previous_unique_id` — `_async_clear_previous_unique_ids` in `__init__.py` räumt dieses Feld direkt danach leer, aber HA-Backups, die vor diesem Fix angelegt wurden, können die Klartext-IBAN dort noch enthalten. `core.entity_registry` liegt außerdem mit Mode 0644 vor, anders als die eigenen Stores der Integration (0600). *Zweites Restrisiko:* Dieselbe Migration löst pro umgeschriebener Entity ein `entity_registry_updated`-Event aus, dessen `changes`-Payload den **alten**, unverschlüsselten `unique_id`-Wert trägt — also bei einer Migration von einer Vor-Hash-Version die Klartext-IBAN. Dieses Event landet auf dem Event-Bus und wird vom Recorder in `home-assistant_v2.db` (`event_data.shared_data`) mitgeschrieben; die Integration kann das nicht unterdrücken, da HA das Event selbst feuert. Betroffen sind ausschließlich Updates von einer Installation mit noch nicht migrierten (Klartext-IBAN-)`unique_id`s — ein Neuinstall durchläuft diese Migration nie und erzeugt daher auch kein solches Event. Die Zeile altert mit dem Recorder-Purge-Fenster aus (Default `purge_keep_days: 10`), aber jedes HA-Backup, das innerhalb dieses Fensters gezogen wird, enthält sie mit — siehe dazu auch die Backup-Empfehlung in §8 und unter „Empfehlungen für Betreiber".
...
Der Config-Flow ist inzwischen bei `VERSION = 3`: die `unique_id` der Config-Entry war früher der Klartext `"{blz}_{username}"`, der damit im Config-Entry-Datensatz (`.storage/core.config_entries`) landete — und damit auch in jedem unverschlüsselten Backup und in Diagnose-Downloads dieser Datei. Der v2→v3-Schritt ersetzt ihn durch `_entry_unique_id(key, blz, username)` — einen **HMAC-SHA256 mit dem Master-Fernet-Key** als Schlüssel, auf 16 Hex-Zeichen gekürzt. Als Nachricht dient `entry_unique_id|{blz}|{username}`; das Präfix ist Domain-Separation, damit diese Verwendung des Master-Keys sich nicht mit späteren keyed Verwendungen ins Gehege kommt.

Ein *ungeschlüsselter* Hash würde hier nicht genügen: die BLZ steht im selben `core.config_entries` im Klartext, und NetKey-Logins sind kurze numerische Kundennummern. Wer die Datei lesen kann, könnte einen unsalted SHA-256 über BLZ+Login also offline in Minuten durchprobieren und das Login rekonstruieren. Mit HMAC scheidet dieser Offline-Brute-Force aus, weil der Schlüssel in `.storage/fints_atruvia_master_key` (Mode 0600) liegt und weder in `entry.data` noch sonst irgendwo im Config-Entry-Datensatz auftaucht. Für die Deduplizierung reicht das: `_abort_if_unique_id_configured` muss dasselbe BLZ/Login-Paar nur innerhalb *einer* HA-Installation wiedererkennen, und dort ist der Master-Key persistent. (Auf HA 2026.3.2 verifiziert: Die gehashte `unique_id` wird auch von `config_entries/get`, `config_entries/get_single` und der REST-Entsprechung nicht ausgeliefert, da `ConfigEntry.as_json_fragment` kein `unique_id`-Feld kennt — das ist eine Beobachtung zu dieser HA-Version, keine dauerhafte Garantie für künftige Releases. Der HMAC ist davon unabhängig begründet: allein die Exposition in `.storage/core.config_entries` und Backups reicht dafür aus.)
